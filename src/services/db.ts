...
export interface FullAppDataBackup {
  ...
  stores: {
    ...
    outlineHistory: any[];
    styleHistory: any[];
    xpPreferencesHistory: any[];
    inspirationRecords: InspirationRecord[];
  };
}

export async function exportAllAppData(): Promise<FullAppDataBackup> {
  const books = await dbGetAll<Book>(STORE_NAMES.BOOKS);
  const configsWrapper = await dbGet(STORE_NAMES.CONFIGS, 'all');
  const uiSettings = await dbGet<UiSettings>(STORE_NAMES.UI_SETTINGS, 'current');
  const personaHistory = await dbGetAll(STORE_NAMES.PERSONA_HISTORY);
  const outlineHistory = await dbGetAll(STORE_NAMES.OUTLINE_HISTORY);
  const styleHistory = await dbGetAll(STORE_NAMES.STYLE_HISTORY);
  const xpPreferencesHistory = await dbGetAll(STORE_NAMES.XP_PREFERENCES_HISTORY);
  const inspirationRecords = await dbGetAll<InspirationRecord>(STORE_NAMES.INSPIRATION_RECORDS);
  ...
  return {
    ...
    stores: {
      ...
      outlineHistory,
      styleHistory,
      xpPreferencesHistory,
      inspirationRecords,
    },
  };
}

export async function importAllAppData(backup: FullAppDataBackup): Promise<void> {
  if (!backup || !backup.stores) {
    throw new Error('备份数据格式不正确，无法还原');
  }

  const { stores } = backup;

  // Clear existing data in stores
  await dbClear(STORE_NAMES.BOOKS);
  await dbClear(STORE_NAMES.CONFIGS);
  await dbClear(STORE_NAMES.UI_SETTINGS);
  await dbClear(STORE_NAMES.PERSONA_HISTORY);
  await dbClear(STORE_NAMES.OUTLINE_HISTORY);
  await dbClear(STORE_NAMES.STYLE_HISTORY);
  await dbClear(STORE_NAMES.XP_PREFERENCES_HISTORY);
  await dbClear(STORE_NAMES.INSPIRATION_RECORDS);

  // Restore Books
  if (Array.isArray(stores.books)) {
    for (const book of stores.books) {
      await dbPut(STORE_NAMES.BOOKS, book);
    }
  }

  // Restore Configs
  if (stores.configsWrapper) {
    await dbPut(STORE_NAMES.CONFIGS, stores.configsWrapper);
  }

  // Restore UI Settings
  if (stores.uiSettings) {
    await dbPut(STORE_NAMES.UI_SETTINGS, stores.uiSettings);
  }

  // Restore Histories
  if (Array.isArray(stores.personaHistory)) {
    for (const item of stores.personaHistory) await dbPut(STORE_NAMES.PERSONA_HISTORY, item);
  }
  if (Array.isArray(stores.outlineHistory)) {
    for (const item of stores.outlineHistory) await dbPut(STORE_NAMES.OUTLINE_HISTORY, item);
  }
  if (Array.isArray(stores.styleHistory)) {
    for (const item of stores.styleHistory) await dbPut(STORE_NAMES.STYLE_HISTORY, item);
  }
  if (Array.isArray(stores.xpPreferencesHistory)) {
    for (const item of stores.xpPreferencesHistory) await dbPut(STORE_NAMES.XP_PREFERENCES_HISTORY, item);
  }
  if (Array.isArray(stores.inspirationRecords)) {
    for (const item of stores.inspirationRecords) await dbPut(STORE_NAMES.INSPIRATION_RECORDS, item);
  }
}
